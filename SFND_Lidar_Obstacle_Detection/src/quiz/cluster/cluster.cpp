...
void find_nearby_points(const std::vector<std::vector<float>>points, int point_idx, KdTree * tree, float distanceTol, 
		std::vector<bool> &points_processed, std::vector<int> &cluster)
{
	
	points_processed[point_idx]	= true;
	cluster.push_back(point_idx);

	//for (int i = 0; i < points_processed.size(); i++)
	//   printf("(%d) ", (int) points_processed[i]);



	std::vector<int> nearby = tree->search(points[point_idx], distanceTol);

	for (int nearbyId : nearby)  // iterate through the nearby Ids from the search function
	{

		if (points_processed[nearbyId] == false)
		{
			//printf("<%d>", nearby[nearby_idx]);			
			find_nearby_points(points, nearbyId, tree, distanceTol, points_processed, cluster);
		}
...
std::vector<std::vector<int>> euclideanCluster(const std::vector<std::vector<float>> points, KdTree* tree, float distanceTol)
{
	...
	for (point_idx = 0; point_idx < points.size(); point_idx++)
	{
		if (points_processed[point_idx] == false)
		{
			cluster.clear();
			find_nearby_points(points, point_idx, tree, distanceTol, points_processed, cluster);
